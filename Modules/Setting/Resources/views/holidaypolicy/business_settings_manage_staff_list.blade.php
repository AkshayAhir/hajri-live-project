...
@section('title')
<title>{{env('APP_NAME')}} | Holiday Staff List</title>
@endsection
@section('header-page')
...
@endsection
@section('content')
<div class="main-content-inner on-time-main main-atten leave_manage_list">

    <row>
        <div class="holiday-policy-edit-sec">

            <div class="weekly-top-nots">
                <h2 class="proxima_nova_semibold">Apply Template To Staff</h2>
                <p class="">{{$holiday_template['name']}}</p>
                <span class="staff-with-holiday">Staff With {{$holiday_template['name']}}</span>
            </div>
        </div>

        <div class="approve-datas atten-datas">
            <div class="business-managedata-search assign-data-search col-md-6 col-sm-6 atten-data-search ">
                <div class="business-staff-count">
                    <p class="proxima_nova_semibold holiday-staff-list-day">Total Staffs
                    <p class="holiday-policy-count proxima_nova_bold">({{$staff_count}})</p>
                    </p>
                </div>
                <form action="" method="">
    ...
    <div class="approve_staff_data manage_staff_list_table">
    ...
    <div class="assign-edit-btn">
        <button name="" id="apply_staff" class="save-staff-btn proxima_nova_semibold">Apply (<span class="save_count">0</span>)
...
<script>
    ...
    $(document).ready(function () {
        ...
        function datatable(searchValue = null) {
            table = $('#staff_datas').DataTable({
                ...
                ajax: {
                    "url": "{{ route('holiday_staff_list') }}",
                    "dataType": "json",
                    "type": "POST",
                    "data": { _token: "{{csrf_token()}}", 'searchValue': searchValue, 'id': <?php echo $holiday_template['id']?>}
                },
                columns: [
                    {
                        data: 'id',
    ...
    $('#apply_staff').on('click', function () {
        var id = <?php echo $holiday_template['id']?>;
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            type: 'POST',
            url: "{{ route('holiday_to_staff') }}",
            data: {
            ...
            success: function (response) {
                if (response['status'] == 1) {
                    $('.loader').show();
                    $('#apply_staff').prop('disabled', true);
                    selectedIds = [];
                    unselectedIds = [];
