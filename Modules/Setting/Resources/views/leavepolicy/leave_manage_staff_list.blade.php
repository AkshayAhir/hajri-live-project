...
@section('title')
    <title>{{env('APP_NAME')}} | Leave Policy Staff List</title>
@endsection
@section('header-page')
...
@section('content')
    <div class="main-content-inner on-time-main main-atten leave_manage_list">
            ...
            <row>
                <div class="approve-datas atten-datas">
                    <div class="atten-data-search assign-data-search col-md-6 col-sm-6">
                        <p class="proxima_nova_semibold holiday-staff-list-day">{{$leave_template['template_name']}}</p>
                        <form action="" method="">
        ...
        <div class="approve_staff_data manage_staff_list_datatable">
...
@section('scripts')
    <script>
        ...
        $(document).ready(function (){
            ...
            function datatable(searchValue = null) {
                table = $('#staff_datas').DataTable({
                    ...
                    ajax: {
                        "url": "{{ route('leave_to_staff_list') }}",
                        "dataType": "json",
                        "type": "POST",
                        "data": {_token: "{{csrf_token()}}",'searchValue':searchValue,'id': <?php echo $leave_template['id']?>}
                    },
                    columns: [
                        {
                            data: 'id',
        ...
        $('#apply_staff').on('click',function(){
            var id = <?php echo $leave_template['id']?>;            
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: 'POST',
                url: "{{ route('leave_to_staff') }}",
                data: {
