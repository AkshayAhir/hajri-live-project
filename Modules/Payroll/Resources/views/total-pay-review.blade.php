            ...
            <div class="top-header-sub">
                <div class="breadcrumbs-area-details clearfix">
                    ...
                    <a class="back_button">
                        ...
                        <h4 class="page-title pull-left proxima_nova_semibold">Total Pay Review
                        </h4>
                    </div>
                    </div>
                    <ul class="breadcrumbs pull-left">
                        <li class="section_sub_title"><a href="#">Payroll</a></li>
                        <li class="section_sub_title">/ Reviews Details</li>
                        <li class="section_sub_title">/ Total Pay Review</li>
                    </ul>
                </div>
            </div>
        </div>
...
@section('content')
        ...
        <div class="main-payroll-sec">
            <div class="shifts-data">
                <div class="filters sidebar-select approve-punch-data">
                    <select id="main_select_services" class="form-select create-select section_sub_title select-club-services" name="club-services">
    ...
    <div class="approve-main-content">
        <div class="approve_staff_data reports_attendance_table review-table">
            <table id="staff_datas" class="payroll_process_datas display dataTable no-footer dtr-inline" style="width: 100%;">
                <thead>
                    <tr>
...
@section('scripts')
<script>
    $('#approve_step2_btn').click(function() {
        localStorage.setItem('step2-next-button', 'true');
        window.location.href = 'payroll-approvals'; 
    });
        var table;
        var selectedIds = [];
        function datatable(searchValue = null,department_id = null) {
            table = $('.payroll_process_datas').DataTable({
                ...
                ajax: {
                    "url": "{{ route('all-total-pay-review') }}",
                    "dataType": "json",
                    "type": "POST",
        ...
        function selectAllCheckboxes() {
            $('.selectCheckbox_model').prop('checked', $('#selectAllCheckbox').prop('checked'));
        }
        ...
        $(document).on('click', '.selectDropdown ul li', function() {
            var department_id = $(this).find('a').data('id'); // Retrieve the data-id attribute from the <a> element inside the <li>
            var searchValue = null;
            table.destroy();
            datatable(searchValue,department_id);
        });
